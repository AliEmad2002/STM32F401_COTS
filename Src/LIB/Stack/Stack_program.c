...
void Stack_voidPush(void* ptr)
{
	...
	STACK_COPY_FUNCTION(stack[stackCount++], (STACK_TYPE_PTR)ptr);
...
void Stack_ptrPop(void* ptr)
{
	...
	STACK_COPY_FUNCTION((STACK_TYPE_PTR)ptr, stack[0]);

	/*	free the first of the stack	*/
	STACK_FREE_FUNCTION(stack[0]);
