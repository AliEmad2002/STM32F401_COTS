 ...
 */
void Stepper_voidInit(Stepper_t* stepperPtr, GPIO_Pin_t _stepPin, GPIO_Pin_t _dirPin)
{
	/*	init pins	*/
 ...
 */
void Stepper_voidStep(Stepper_t* stepperPtr, Stepper_Direction_t dir, u64 currentTime)
{
	/*	output prober direction signal	*/
	switch (dir)
	{
		case Stepper_Direction_forward:
			GPIO_voidSetPinOutputLevel(stepperPtr->dirPort, stepperPtr->dirPin, GPIO_OutputLevel_High);
			stepperPtr->currentPos++;
			break;
		default:	//	i.e.: Stepper_dir_backward:
			GPIO_voidSetPinOutputLevel(stepperPtr->dirPort, stepperPtr->dirPin, GPIO_OutputLevel_Low);
			stepperPtr->currentPos--;
	}
	
	/*	step	*/
	GPIO_voidSetPinOutputLevel(stepperPtr->stepPort, stepperPtr->stepPin, GPIO_OutputLevel_High);
	Delay_voidDelay2Us();
	GPIO_voidSetPinOutputLevel(stepperPtr->stepPort, stepperPtr->stepPin, GPIO_OutputLevel_Low);
