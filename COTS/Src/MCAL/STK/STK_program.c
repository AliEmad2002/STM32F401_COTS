...
void STK_voidInit(STK_ClockSource_t clkSource, u32 preload)
{
	WRT_BIT(STK->CTRL, STK_CLKSOURCE, clkSource);
	STK->LOAD = preload;
	STK->VAL = 0;
}
...
u32 STK_u32GetTicksPerSecond(void)
{
	...
	u32 stkOver8 = STK_b8IsTickDivBy8() ? 8 : 1;
...
{
	return !GET_BIT(STK->CTRL, STK_CLKSOURCE);
}
