...
void RCC_voidInitPLL(void)
{
	/*	setting PREDIV1 (if chosen as PLL source)	*/
	#if RCC_PLL_CLK_SOURCE == RCC_PLLSOURCE_PREDIV1
	/*	setting PREDIV1 source	*/
	WRT_BIT(RCC->CFGR2, RCC_PREDIV1SRC, RCC_PREDIV1SOURCE_HSE);	//	CHANGE IT NEXT VERSION TO BE CONFIGURABLE.

	/*	setting PREDIV1 value	*/
	EDT_REG(RCC->CFGR2, RCC_PREDIV1_0, RCC_PREDIV1_PRESCALER-1, 4);
	#endif

	/*	setting PLL clock source	*/
	WRT_BIT(RCC->CFGR, RCC_PLLSRC, RCC_PLL_CLK_SOURCE);

	/*	setting PLLMUL factor	*/
	EDT_REG(RCC->CFGR, RCC_PLLMUL_0, RCC_PLLMUL, 4);
}
#endif
